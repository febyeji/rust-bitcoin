...
mod tests {
    ...
    fn static_vector() {
        ...
        let block: Block = encoding::decode_from_slice(&segwit_block[..]).expect("failed to deserialize block");
    ...
    fn coinbase_and_bip34() {
        ...
        let block: Block = encoding::decode_from_slice(&hex!(BLOCK_HEX_SMALL_HEIGHT_42_WRONG)).unwrap();
        let block = block.assume_checked(None);
    ...
    fn block() {
        ...
        let unchecked = Block::new_unchecked(*real_decode.header(), real_decode.transactions().to_vec());
        assert_eq!(encoding::encode_to_vec(&unchecked), some_block);
    }
    ...
    fn segwit_block() {
        ...
        let unchecked = Block::new_unchecked(*real_decode.header(), real_decode.transactions().to_vec());
        assert_eq!(encoding::encode_to_vec(&unchecked), segwit_block);
    }
    ...
    fn validate_pow() {
        ...
        let some_header: Header =
            encoding::decode_from_slice(&some_header).expect("can't deserialize correct block header");
        assert_eq!(
